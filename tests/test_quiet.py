...
def bilan_par_categorie(agent, testset, output_file=None):
    ...
    for case in testset:
        ...
        for cat in CATEGORIES:
            ...
            # Count matches/mismatches for each category
    ...
    # Summary statistics
    lines.append("\n=== PER-CATEGORY SUMMARY (FN/FP/TP) ===")
    for cat in CATEGORIES:
        d = stats[cat]
        lines.append(f"{cat.capitalize():11}: TP={d['TP']:2} | FN={d['FN']:2} | FP={d['FP']:2}")
    lines.append("\n--- Top FN/FP examples per category ---")
    for cat in CATEGORIES:
        if details[cat]:
            lines.append(f"\n{cat.upper()}:")
            for entry in details[cat][:5]:  # Only show the first 5 mismatches per category
...
def main():
    # Path to the testset file
    testset_path = os.path.join(os.path.dirname(__file__), 'testsets', 'matching_eval.json')    
    output_file = "quiet_focus_eval.txt"
    print(f"[INFO] Loading testset: {testset_path}")
    print(f"[INFO] Detailed results will also be written to {output_file}")
    ...
    # Count exact matches across all categories simultaneously
    exact_match = 0
    for case in testset:
        query = case["query"]
        expected = case.get("expected_focus_quiet", {})
    ...
    # Detailed per-category evaluation (printed and saved)
